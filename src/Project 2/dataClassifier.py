...
def enhancedFeatureExtractorDigit(datum):
    ...
    ## DESCRIBE YOUR ENHANCED FEATURES HERE...
        our features are the basicFeatureExtractorDigit and added 
        new features according to cell last 3 neighbours. features(i,j,k),
        where k is up, down or up_left, is pixel difference
        between cell(i,j) and one of its neighbour cells, left one,
        up one or left than up one.
                X X *    for #(i,j) we take the
                X # *    last 3 cells(X) and save
    ...
    "*** YOUR CODE HERE ***"
    for i in range(1, DIGIT_DATUM_WIDTH-1):
        for j in range(1, DIGIT_DATUM_HEIGHT-1):
            for k in [(i-1,j,'up'),(i,j-1,'left'),(i-1,j-1,'up_left')]:
                features[(i,j,k[2])] = datum.getPixel(i,j) - datum.getPixel(k[0],k[1])
...
def enhancedPacmanFeatures(state, action):
    ...
    features = util.Counter()
    "*** YOUR CODE HERE ***"
    # feature for StopAgent
    if action == 'STOP': features['stop'] = sys.maxint

    if state.isWin(): features['win'] = sys.maxint
    if state.isLose(): features['lose'] = -sys.maxint

    successor = state.generatePacmanSuccessor(action)

    # returns top n closest points list from pacman by manh.distance.
    def min_distances(pacman_pos, positions, top = 7):
        dist = util.Counter()
        for p in positions:
            dist[p] = -util.manhattanDistance(p, pacman_pos)
        sorted_dist = [-dist[p] for p in dist.sortedKeys()]
        return sorted_dist[:min(top,len(sorted_dist))]

    # weights are choosen according to Agents prioritys.

    # feature for FoodAgent and take it with some weight (2pi)
    food = state.getFood().asList()
    for closest_food in min_distances(successor.getPacmanPosition(), food):
        features['closest_food'] += 2*math.pi / (1.0 + closest_food)

    # feature for SuicideAgent and take it with 2*FoodAgent.weight (4pi)
    ghosts = successor.getGhostPositions()
    for closest_ghost in min_distances(successor.getPacmanPosition(), ghosts):
        features['closest_ghost'] += 4*math.pi / (1.0 + closest_ghost)

    # feature for ContestAgent and take it with 3*FoodAgent.weight (6pi)
    capsules = successor.getCapsules()
    for closest_capsule in min_distances(successor.getPacmanPosition(), capsules):
        features['closest_capslue'] += 6*math.pi / (1.0 + closest_capsule)
