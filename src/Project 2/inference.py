...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            # count real distance between legal position and pacman
            trueDistance = util.manhattanDistance(p, pacmanPosition)
            # count B(x[t]) oc(proportional) P(e[t]|x[t])*B'(x[t])
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            # get distr. after fixing the gohst at position p
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            #B'(X[t]) = E(sum x[t-1])P(X[t]|x[t-1])*B(x[t-1])
            for newPos, prob in newPosDist.items():
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        self.particles = [] # fill this with legalPostions until size != numParticles
        while len(self.particles) < self.numParticles:
            self.particles += self.legalPositions
        
        # cut the extra partocles, if there are.
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        # returns beliefes according to the particles distr
        # doing same as ExactInference observe - updates belifes
        # but for doing less operations do it without belifes.
        def getBeliefs():
            ...
            # this will be same as if at first we counted distributions
            # of uniq positions in particles and than iterate over them
            # and take them with their prob./weight: allPos[p] would be
            # emissionModel[trueD]*belifes[p], but here no need belifes[p]
            # because there are multiple times each pos. and normalizetion
            # will do same. 
            for p in self.particles:
                # count the real dist between part nad pacman
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                # add probablity if its available to reach here from other point
                # and after normalize it.
                allPossible[p] += emissionModel[trueDistance]*1.0
            
            # need to normalize probabilitys!
            allPossible.normalize()
            return allPossible
            
            
        self.beliefs = getBeliefs()
        # handle if all weights are 0 and recreate the priror distr.
        if self.beliefs.totalCount() == 0:
            self.initializeUniformly(gameState)
            return

        # update particles faster, resample accordint to updated beliefes...
        self.particles = [a[0] for a in util.nSample(self.beliefs.values(), self.beliefs.items(), self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        new_particles = []
        # we should make new particles according to particles
        # it will do/be same as if we at first count belifes and than 
        # resample according to it; the proprtion will be held here
        # because belifes is created by particles.
        for p in self.particles:
            # get distr. after fixing the ghost at position p
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, p))
            # add new sample - created according to newPosDist
    ...
    def getBeliefDistribution(self):
        ...
        # count the frequency of parts in particles
        for part in self.particles:
            beliefs[part] += 1.0
        
        # normalize belifes to get real probabilitys.
        beliefs.normalize()
        return beliefs
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        # make all possible permutations with length numGhosts and than shuffle them randomly
        all_permutation = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(all_permutation)
        
        self.particles = [] # initialize particles with all_permutation legal postions
        while len(self.particles) < self.numParticles:
            self.particles += all_permutation
        
        # cut the extra parts to fix size on numParticles.
        self.particles = self.particles[:self.numParticles]  
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # returns belifes according to the jailed-particles
        # according to their likelihood weights 
        def getBeliefs():
            jailed_idxs = []
            allPossible = util.Counter()
            # save indexes of jailed ghosts
            for idx_noisy in enumerate(noisyDistances):      
                if idx_noisy[1] is None: jailed_idxs.append(idx_noisy[0])

            # "jail ghost" - update and return part, if ghost is in jail.
            def JailParticle(part):
                for idx in jailed_idxs:
                    part = self.getParticleWithGhostInJail(part, idx)
                return part

            # updated particles - handling jailed ghosts
            jailed_particles = [JailParticle(part) for part in self.particles]
            for part in jailed_particles:
                prob = 1.0
                for pos_idx in range(self.numGhosts):
                    # if ghost is jailed - no need update
                    if noisyDistances[pos_idx] is None: continue

                    # count probabilitys according their weights
                    trueDistance = util.manhattanDistance(part[pos_idx], pacmanPosition)
                    # calculate the weight how this prob. will take place in beliefs.
                    prob *= emissionModels[pos_idx][trueDistance]
                allPossible[part] += prob
        ...
        # update particles according to updated beliefs
        self.particles = [a[0] for a in util.nSample(self.beliefs.values(), self.beliefs.items(), self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            "*** YOUR CODE HERE ***"
            # elpaseTime for all ghosts 
            for i in range(self.numGhosts):
                # get new distribution for each ghost after setting ghosts according to old postions 
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, list(oldParticle)), i, self.ghostAgents[i])
                # update-resample each gohst's particles according to new distributions
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        beliefs = util.Counter()
        # count how many times each particle occurs and than normalize.
        for part in self.particles:
            beliefs[part] += 1.0
